...
%               0:5:175.
%   p           Number of parallel rays for each angle. Default: p = 2*N.
...
%                   dimension: (p*length(theta))
%                   true form: b_proj=reshape(b,[p,length(theta)]); b_proj=reshape(b,[128,36]);
%                   show: imshow(b_proj,[])
%   x           Vector containing the exact solution, with elements
%               between 0 and 1.
%                   true form: x_proj=reshape(xex,[p,length(theta)]);
%                   show: imshow(x_proj,[])
